...
function getOffscreenOceanTile () {
    tileList = tiles.getTilesByType(assets.tile`transparency16`)
    tileToCheck = tileList.removeAt(randint(0, tileList.length - 1))
    console.log("Start")
    console.log(tileList)
    console.log(tileToCheck)
    // console.log(tiles)
    // console.log(tiles.XY)
    console.log(dolphinBody)
    // console.log(tiles.locationXY(tileToCheck, tiles.XY.right))
...
let title = sprites.create(img`
    ...
    `, SpriteKind.Player)
pause(1000)
game.showLongText("Move with WASD or the ARROW KEYS. Jump above the water and press SPACE while falling to do a trick. Bad tricks are the only way of gaining points. Doing tricks will drain your health. Your health slowly recharges over time. You have 60 seconds. Press SPACE to continue.", DialogLayout.Full)
title.destroy()
titleHidden = true
...
tiles.placeOnRandomTile(dolphinBody, assets.tile`tile8`)
